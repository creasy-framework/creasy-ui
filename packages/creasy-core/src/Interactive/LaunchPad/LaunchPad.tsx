...
const LaunchPad: LaunchPadComponent = ({
  ...
  const defaultTrigger = (
    <Avater
      ...
      className={classNames(
        'creasy-launchpad__trigger',
        {
          ['creasy-launchpad__trigger--expanded']: isExpanded,
        },
      )}>
      <Icons.Add/>
    </Avater>
  ...
  return (
    <div className="creasy-launchpad">
      {
        useOverlay && (
          <div
            className={classNames('creasy-launchpad-overlay', isExpanded && 'creasy-launchpad-overlay--show')}
            onClick={() => setIsExpanded(false)}
          >
          </div>
        )
      }
      <div className="creasy-launchpad__trigger-wrapper" onClick={() => setIsExpanded(!isExpanded)}>
        {trigger}
      </div>
      {
        React.Children.map(children, (child, i) => (
          <span
            className="creasy-launchpad-item-container"
